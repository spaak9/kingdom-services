...
function convertArabicNumbers(value: string) {
  ...
  return value
    .replace(/[٠-٩]/g, (number) =>
      String(arabicNumbers.indexOf(number)),
    )
    .replace(/[۰-۹]/g, (number) =>
      String(persianNumbers.indexOf(number)),
    );
}

function getDigits(value: string) {
  return convertArabicNumbers(value).replace(/\D/g, "");
}
...
async function getGoogleMapsEmbedUrl(value: string) {
  ...
  try {
    ...
    if (!isGoogleMapsHost(original.hostname)) {
      return "";
    }

    if (
      original.hostname.endsWith("google.com") &&
      original.pathname.includes("/maps/embed")
    ) {
      return originalUrl;
    }
    ...
    if (
      ...
      try {
        const response = await fetch(originalUrl, {
          ...
          headers: {
            "User-Agent":
              "Mozilla/5.0 (compatible; KingdomServices/1.0)",
          },
        });
        ...
        console.error(
          "Failed to resolve Google Maps URL:",
          error,
        );
      }
    }
    ...
    const placeMatch = decodeURIComponent(
      resolved.pathname,
    ).match(/\/maps\/place\/([^/@]+)/);
...
export async function generateMetadata({
  ...
  return {
    title: `${seoTitle} | المملكة للخدمات المنزلية`,
    description,
    keywords: [
      seoTitle,
      `${service.searchName} في ${city.name}`,
      `${service.pluralName} ${city.name}`,
...
export default async function ServiceCityPage({
  ...
  const savedContact =
    contactData as ServiceContact | null;

  if (contactError) {
    console.error(
      "Failed to load service contact:",
      contactError,
    );
  }

  const contactIsActive =
    savedContact?.is_active ?? true;

  const hasSavedContact = Boolean(savedContact);

  const boxOneValue = hasSavedContact
    ? savedContact?.whatsapp_number?.trim() || ""
    : WHATSAPP_NUMBER;

  const boxTwoValue = hasSavedContact
    ? savedContact?.phone_number?.trim() || ""
    : "0598863130";

  const boxOneIsPhone = isPhoneValue(boxOneValue);
  const boxTwoIsPhone = isPhoneValue(boxTwoValue);

  const whatsappNumber = boxOneIsPhone
    ? toWhatsAppNumber(boxOneValue)
    : "";

  const internationalPhone = boxTwoIsPhone
    ? toInternationalPhone(boxTwoValue)
    : "";

  const googleMapsUrl =
    savedContact?.google_maps_url?.trim() || "";
  ...
  const whatsappMessage = encodeURIComponent(
    `السلام عليكم، أرغب في الاستفسار عن خدمة ${service.searchName} في ${city.name}.`,
  );
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    name: seoTitle,
    serviceType: service.pluralName,
    ...
    availableChannel: {
      ...
      servicePhone: internationalPhone || undefined,
    },
  };
  ...
  return (
    ...
    >
      ...
      <main>
        <section className="relative isolate overflow-hidden px-4 py-14 text-center md:py-20">
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <h1 className="mx-auto mt-5 max-w-4xl text-[38px] font-black leading-[1.3] md:text-5xl lg:text-6xl">
              {seoTitle}
            </h1>
            ...
            {contactIsActive ? (
              <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                {boxOneValue &&
                  (boxOneIsPhone && whatsappUrl ? (
                    <a
                      href={whatsappUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex min-h-13 w-full items-center justify-center rounded-2xl bg-[#25D366] px-6 text-sm font-black text-white shadow-[0_14px_35px_rgba(37,211,102,0.25)] transition hover:-translate-y-0.5 sm:w-auto"
                    >
                      {boxOneValue}
                    </a>
                  ) : (
                    <div className="flex min-h-13 w-full cursor-default items-center justify-center rounded-2xl bg-[#25D366]/20 px-6 text-sm font-black text-[#78f0a4] sm:w-auto">
                      {boxOneValue}
                    </div>
                  ))}

                {boxTwoValue &&
                  (boxTwoIsPhone && internationalPhone ? (
                    <a
                      href={`tel:${internationalPhone}`}
                      className="flex min-h-13 w-full items-center justify-center rounded-2xl border border-[#e8ad45]/50 bg-[#e8ad45]/10 px-6 text-sm font-black text-[#f3c36f] transition hover:bg-[#e8ad45] hover:text-[#031225] sm:w-auto"
                    >
                      {boxTwoValue}
                    </a>
                  ) : (
                    <div className="flex min-h-13 w-full cursor-default items-center justify-center rounded-2xl border border-[#e8ad45]/30 bg-[#e8ad45]/10 px-6 text-sm font-black text-[#f3c36f] sm:w-auto">
                      {boxTwoValue}
                    </div>
                  ))}
              </div>
            ) : (
              <div className="mx-auto mt-8 max-w-md rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4 text-sm font-bold text-white/60">
        ...
        <section className="bg-[#06182e] px-4 py-14 md:py-20">
          <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <article className="rounded-3xl border border-white/10 bg-[#081d37] p-6 shadow-2xl md:p-9">
              <span className="text-sm font-bold text-[#e8ad45]">
                تفاصيل الخدمة
              </span>

              <h2 className="mt-3 text-3xl font-black">
                {service.pluralName} في {city.name}
              </h2>

              <p className="mt-5 text-[16px] leading-9 text-white/68">
                نوفر في المملكة للخدمات المنزلية صفحة مستقلة لكل خدمة ومدينة،
                حتى تكون المعلومات واضحة ومباشرة. تعرض هذه الصفحة خدمات{" "}
                {service.name} في {city.name}، مع وسيلة تواصل مباشرة للاستفسار
                عن الأعمال المتاحة والتفاصيل المتعلقة بالخدمة.
              </p>
              ...
              <div className="mt-7 grid gap-3 sm:grid-cols-2">
                {service.details.map((detail) => (
                  <div
                    key={detail}
                    className="flex min-h-20 items-center gap-3 rounded-2xl border border-white/10 bg-[#031225]/70 p-4"
                  >
            ...
            <aside className="rounded-3xl border border-[#e8ad45]/20 bg-[#081d37] p-6 shadow-2xl md:p-9">
              <span className="text-sm font-bold text-[#e8ad45]">
                معلومات الصفحة
              </span>
              ...
              {contactIsActive &&
                googleMapsUrl &&
                googleMapsEmbedUrl && (
                  <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-[#031225]">
                    <iframe
                      title={`موقع ${service.searchName} في ${city.name}`}
        ...
        <section className="px-4 py-14 md:py-20">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {relatedServices.map((item) => (
                <Link
                  ...
                  className="rounded-3xl border border-white/10 bg-[#071a31] p-6 text-center transition hover:-translate-y-1 hover:border-[#e8ad45]/50"
                >
            ...
            {nearbyCities.length > 0 && (
              <>
                <h2 className="mt-14 text-center text-3xl font-black md:text-4xl">
                  {service.pluralName} في مدن قريبة
                </h2>
                ...
                <div className="mt-8 flex flex-wrap justify-center gap-3">
                  {nearbyCities.map((item) => (
                    <Link
                      ...
                      className="rounded-2xl border border-white/10 bg-[#071a31] px-5 py-3 text-sm font-black transition hover:border-[#e8ad45]/50 hover:text-[#e8ad45]"
                    >
                      {service.searchName} {item.name}
                    </Link>
                  ))}
                </div>
              </>
          ...
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mt-8 space-y-3">
              {service.faq.map((item) => (
                <article
                  key={item.question}
                  className="rounded-3xl border border-white/10 bg-[#081d37] p-6"
                >
      ...
      <footer className="border-t border-white/10 bg-[#020d1a] px-4 py-9 text-center">
        <Link href="/">
